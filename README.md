...
Concept
-------
The basic concept is that a JSON object (or document) provides an outline (template) of a report. The template is processed and content (headings, paragraphs, tables of data etc.) are constructed within the DOM into a target element.

Template document format
------------------------
The template defines the external data sources required in a property called `datasources`, any individual variables
(selected from a particular observation from a data source) and an array of `sections` defining the presented content.
...
### datasources
Contained an the `url` property is a series of data sources that will be required by the report and its variables.

The objects must contain:
- `id`: a unique ID property (integer) used to reference the data source from the template sections
- `type`: DAO to use as an adaptor to the data (the DAO must be specified in report parameters)
- `url`: defines the resource location of the data source.

To allow the data source to vary based on the geographic area of the report, the `url` property value may contain a string `$geography` which is substituted with a value specified in parameters at time of report initalisation.
